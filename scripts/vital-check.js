...
const vitalNextPath = 'packages/c2';
...
if (vitalHash !== vitalNextHash) {
  ...
  // Log file missing from c2
  vitalMoreFiles.forEach(file => differences.push(
    `${RED}\t--vital-- contains the file ${file}, it is missing from c2.${NC}`
  ));

  // Log file missing from --vital--
  vitalNextMoreFiles.forEach(file => differences.push(
    `${RED}\tc2 contains the file ${file}, it is missing from --vital--${NC}`
  ));
  ...
  console.error(`${RED}The packages __vital__ and c2 differ in these files:${NC}\n`);
  differences.forEach(difference => console.error(difference));
  process.exit(-1);
}

console.error(`${GREEN}The packages __vital__ and c2 are identical.${NC}`);
