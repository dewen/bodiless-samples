...
if (vitalHash !== vitalNextHash) {
  ...
  vitalMoreFiles.forEach(file => differences.push(
    `${RED}\tc1 contains the file ${file}, it is missing from --vital-next--.${NC}`
  ));

  // Log file missing from c1
  vitalNextMoreFiles.forEach(file => differences.push(
    `${RED}\t--vital-next-- contains the file ${file}, it is missing from c1${NC}`
  ));
  ...
  console.error(`${RED}The packages c1 and __vital_next__ differ in these files:${NC}\n`);
  differences.forEach(difference => console.error(difference));
  process.exit(-1);
}

console.error(`${GREEN}The packages c1 and __vital_next__ are identical.${NC}`);
